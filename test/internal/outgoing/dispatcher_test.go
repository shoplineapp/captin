...
func TestDispatchEvents_Throttled_With_TrailingEdge(t *testing.T) {
	store, documentStore, sender, dispatcher, throttler := setup("fixtures/config.throttle.enable_trail.json")
	...
	throttler.On("CanTrigger", mock.Anything, mock.Anything).Return(false, 500*time.Millisecond, nil).Once()
...
func TestDispatchEvents_Throttled_Without_TrailingEdge(t *testing.T) {
	store, documentStore, sender, dispatcher, throttler := setup("fixtures/config.throttle.json")
